...
gnome-terminal --tab --title="Hospital Service" -- bash -c "bal run; exec bash" &
sleep 2

# Inventory Service (Port 9091) 
echo -e "${YELLOW}Starting Inventory Service on port 9091...${NC}"
cd ../inventory-service
gnome-terminal --tab --title="Inventory Service" -- bash -c "bal run; exec bash" &
sleep 2

# Search Service (Port 9092)
echo -e "${YELLOW}Starting Search Service on port 9092...${NC}"
cd ../search-service  
gnome-terminal --tab --title="Search Service" -- bash -c "bal run; exec bash" &
sleep 2

# Notification Service (Port 9093)
echo -e "${YELLOW}Starting Notification Service on port 9093...${NC}"
cd ../notification-service
gnome-terminal --tab --title="Notification Service" -- bash -c "bal run; exec bash" &
sleep 2

# Start Frontend
echo -e "${YELLOW}Starting Frontend on port 3000...${NC}"
cd ../../frontend
gnome-terminal --tab --title="MedCura Frontend" -- bash -c "npm run dev; exec bash" &
